...
function cfc_add_metaboxes() {
    // Eventos
    add_meta_box('cfc_evento_details', 'Detalles del Evento', 'cfc_evento_metabox_html', 'cfc_evento', 'normal', 'high');
    // Ministerios
    add_meta_box('cfc_ministerio_details', 'Detalles del Ministerio', 'cfc_ministerio_metabox_html', 'cfc_ministerio', 'normal', 'high');
    // Reflexiones
    add_meta_box('cfc_reflexion_details', 'Detalles de la Reflexión', 'cfc_reflexion_metabox_html', 'cfc_reflexion', 'normal', 'high');
    // Equipo
    add_meta_box('cfc_equipo_details', 'Detalles del Miembro', 'cfc_equipo_metabox_html', 'cfc_equipo', 'normal', 'high');
    // Pages with templates
    add_meta_box('cfc_page_template_fields', 'Opciones de la Página', 'cfc_page_template_metabox_html', 'page', 'normal', 'high');
}
add_action('add_meta_boxes', 'cfc_add_metaboxes');

/**
 * Metabox for Page Templates (Dar, Visítanos, Quiénes Somos)
 */
function cfc_page_template_metabox_html($post) {
    wp_nonce_field('cfc_page_template_save', 'cfc_page_template_nonce');
    $template = get_page_template_slug($post->ID);

    // Get saved values
    $values = array(
        // Dar
        'banco_nombre' => get_post_meta($post->ID, 'banco_nombre', true),
        'banco_cuenta' => get_post_meta($post->ID, 'banco_cuenta', true),
        'banco_titular' => get_post_meta($post->ID, 'banco_titular', true),
        'banco_tipo' => get_post_meta($post->ID, 'banco_tipo', true),
        // Visítanos
        'horario_viernes_nombre' => get_post_meta($post->ID, 'horario_viernes_nombre', true),
        'horario_viernes_hora' => get_post_meta($post->ID, 'horario_viernes_hora', true),
        'horario_viernes_desc' => get_post_meta($post->ID, 'horario_viernes_desc', true),
        'horario_sabado_nombre' => get_post_meta($post->ID, 'horario_sabado_nombre', true),
        'horario_sabado_hora' => get_post_meta($post->ID, 'horario_sabado_hora', true),
        'horario_sabado_desc' => get_post_meta($post->ID, 'horario_sabado_desc', true),
        'galeria_1' => get_post_meta($post->ID, 'galeria_1', true),
        'galeria_2' => get_post_meta($post->ID, 'galeria_2', true),
        'galeria_3' => get_post_meta($post->ID, 'galeria_3', true),
        // Quiénes Somos
        'mision' => get_post_meta($post->ID, 'mision', true),
        'vision' => get_post_meta($post->ID, 'vision', true),
        // Inicio (Homepage)
        'hero_video_url' => get_post_meta($post->ID, 'hero_video_url', true),
        'hero_image_url' => get_post_meta($post->ID, 'hero_image_url', true),
        'hero_badge' => get_post_meta($post->ID, 'hero_badge', true),
        'hero_titulo_1' => get_post_meta($post->ID, 'hero_titulo_1', true),
        'hero_titulo_2' => get_post_meta($post->ID, 'hero_titulo_2', true),
        'adol_titulo' => get_post_meta($post->ID, 'adol_titulo', true),
        'adol_desc' => get_post_meta($post->ID, 'adol_desc', true),
        'adol_edad' => get_post_meta($post->ID, 'adol_edad', true),
        'adol_horario' => get_post_meta($post->ID, 'adol_horario', true),
        'adol_imagen' => get_post_meta($post->ID, 'adol_imagen', true),
        'jov_titulo' => get_post_meta($post->ID, 'jov_titulo', true),
        'jov_desc' => get_post_meta($post->ID, 'jov_desc', true),
        'jov_edad' => get_post_meta($post->ID, 'jov_edad', true),
        'jov_horario' => get_post_meta($post->ID, 'jov_horario', true),
        'jov_imagen' => get_post_meta($post->ID, 'jov_imagen', true),
    );
    ?>
    <style>
        .cfc-template-fields { display: none; }
        .cfc-template-fields.active { display: block; }
        .cfc-template-fields h4 { margin: 20px 0 10px; padding-bottom: 5px; border-bottom: 1px solid #ddd; color: #1e40af; }
        .cfc-template-fields .cfc-metabox-field { margin-bottom: 15px; }
        .cfc-template-fields .cfc-metabox-field label { display: block; font-weight: 600; margin-bottom: 5px; }
        .cfc-template-fields .cfc-metabox-field input,
        .cfc-template-fields .cfc-metabox-field textarea { width: 100%; padding: 8px; }
        .cfc-template-fields .cfc-metabox-field textarea { min-height: 100px; }
        .cfc-no-template { padding: 20px; text-align: center; color: #666; }
    </style>

    <!-- Mensaje cuando no hay template -->
    <div class="cfc-no-template" id="cfc-no-template">
        <p>Selecciona un template (Dar, Visítanos o Quiénes Somos) en el panel derecho para ver las opciones editables.</p>
    </div>

    <!-- Campos para template DAR -->
    <div class="cfc-template-fields" id="cfc-fields-dar">
        <h4>💳 Información Bancaria</h4>
        <div class="cfc-metabox-field">
            <label for="banco_nombre">Nombre del Banco</label>
            <input type="text" id="banco_nombre" name="banco_nombre" value="<?php echo esc_attr($values['banco_nombre']); ?>" placeholder="Ej: Banco General">
        </div>
        <div class="cfc-metabox-field">
            <label for="banco_tipo">Tipo de Cuenta</label>
            <input type="text" id="banco_tipo" name="banco_tipo" value="<?php echo esc_attr($values['banco_tipo']); ?>" placeholder="Ej: Cuenta de Ahorros">
        </div>
        <div class="cfc-metabox-field">
            <label for="banco_cuenta">Número de Cuenta</label>
            <input type="text" id="banco_cuenta" name="banco_cuenta" value="<?php echo esc_attr($values['banco_cuenta']); ?>" placeholder="Ej: 04-47-99-123456-7">
        </div>
        <div class="cfc-metabox-field">
            <label for="banco_titular">Titular de la Cuenta</label>
            <input type="text" id="banco_titular" name="banco_titular" value="<?php echo esc_attr($values['banco_titular']); ?>" placeholder="Ej: Centro Familiar Cristiano">
        </div>
    </div>

    <!-- Campos para template VISITANOS -->
    <div class="cfc-template-fields" id="cfc-fields-visitanos">
        <h4>📅 Horario Viernes (Estudio Bíblico)</h4>
        <div class="cfc-metabox-field">
            <label for="horario_viernes_nombre">Nombre</label>
            <input type="text" id="horario_viernes_nombre" name="horario_viernes_nombre" value="<?php echo esc_attr($values['horario_viernes_nombre'] ?: 'Viernes'); ?>" placeholder="Viernes">
        </div>
        <div class="cfc-metabox-field">
            <label for="horario_viernes_hora">Hora</label>
            <input type="text" id="horario_viernes_hora" name="horario_viernes_hora" value="<?php echo esc_attr($values['horario_viernes_hora'] ?: '7:00 PM'); ?>" placeholder="7:00 PM">
        </div>
        <div class="cfc-metabox-field">
            <label for="horario_viernes_desc">Descripción</label>
            <input type="text" id="horario_viernes_desc" name="horario_viernes_desc" value="<?php echo esc_attr($values['horario_viernes_desc'] ?: 'Estudio Bíblico'); ?>" placeholder="Estudio Bíblico">
        </div>

        <h4>📅 Horario Sábado (Jóvenes)</h4>
        <div class="cfc-metabox-field">
            <label for="horario_sabado_nombre">Nombre</label>
            <input type="text" id="horario_sabado_nombre" name="horario_sabado_nombre" value="<?php echo esc_attr($values['horario_sabado_nombre'] ?: 'Sábado'); ?>" placeholder="Sábado">
        </div>
        <div class="cfc-metabox-field">
            <label for="horario_sabado_hora">Hora</label>
            <input type="text" id="horario_sabado_hora" name="horario_sabado_hora" value="<?php echo esc_attr($values['horario_sabado_hora'] ?: '4:00 PM'); ?>" placeholder="4:00 PM">
        </div>
        <div class="cfc-metabox-field">
            <label for="horario_sabado_desc">Descripción</label>
            <input type="text" id="horario_sabado_desc" name="horario_sabado_desc" value="<?php echo esc_attr($values['horario_sabado_desc'] ?: 'Reunión de Jóvenes'); ?>" placeholder="Reunión de Jóvenes">
        </div>

        <h4>🖼️ Galería de Imágenes</h4>
        <div class="cfc-metabox-field">
            <label for="galeria_1">Imagen 1 (URL)</label>
            <input type="url" id="galeria_1" name="galeria_1" value="<?php echo esc_attr($values['galeria_1']); ?>" placeholder="https://images.unsplash.com/...">
        </div>
        <div class="cfc-metabox-field">
            <label for="galeria_2">Imagen 2 (URL)</label>
            <input type="url" id="galeria_2" name="galeria_2" value="<?php echo esc_attr($values['galeria_2']); ?>" placeholder="https://images.unsplash.com/...">
        </div>
        <div class="cfc-metabox-field">
            <label for="galeria_3">Imagen 3 (URL)</label>
            <input type="url" id="galeria_3" name="galeria_3" value="<?php echo esc_attr($values['galeria_3']); ?>" placeholder="https://images.unsplash.com/...">
        </div>
    </div>

    <!-- Campos para template QUIENES SOMOS -->
    <div class="cfc-template-fields" id="cfc-fields-quienes-somos">
        <h4>🎯 Misión y Visión</h4>
        <div class="cfc-metabox-field">
            <label for="mision">Nuestra Misión</label>
            <textarea id="mision" name="mision" placeholder="Escribe la misión de la iglesia..."><?php echo esc_textarea($values['mision']); ?></textarea>
        </div>
        <div class="cfc-metabox-field">
            <label for="vision">Nuestra Visión</label>
            <textarea id="vision" name="vision" placeholder="Escribe la visión de la iglesia..."><?php echo esc_textarea($values['vision']); ?></textarea>
        </div>
        <p class="description">El equipo de líderes se edita desde el CPT "Equipo" en el menú lateral.</p>
    </div>

    <!-- Campos para template INICIO (Homepage) -->
    <div class="cfc-template-fields" id="cfc-fields-inicio">
        <h4>🎬 Hero Section</h4>
        <div class="cfc-metabox-field">
            <label for="hero_video_url">URL del Video (MP4)</label>
            <input type="url" id="hero_video_url" name="hero_video_url" value="<?php echo esc_attr($values['hero_video_url']); ?>" placeholder="https://ejemplo.com/video.mp4">
        ...
        <div class="cfc-metabox-field">
            ...
            <input type="url" id="hero_image_url" name="hero_image_url" value="<?php echo esc_attr($values['hero_image_url']); ?>" placeholder="https://images.unsplash.com/...">
            <p class="description">Imagen de fondo si no hay video. Si ambos están vacíos, usará gradiente.</p>
        </div>
        <div class="cfc-metabox-field">
            <label for="hero_badge">Texto del Badge</label>
            <input type="text" id="hero_badge" name="hero_badge" value="<?php echo esc_attr($values['hero_badge'] ?: 'En vivo cada domingo'); ?>" placeholder="En vivo cada domingo">
        </div>
        <div class="cfc-metabox-field">
            <label for="hero_titulo_1">Título Línea 1</label>
            <input type="text" id="hero_titulo_1" name="hero_titulo_1" value="<?php echo esc_attr($values['hero_titulo_1'] ?: 'Centro Familiar'); ?>" placeholder="Centro Familiar">
        </div>
        <div class="cfc-metabox-field">
            <label for="hero_titulo_2">Título Línea 2 (destacado)</label>
            <input type="text" id="hero_titulo_2" name="hero_titulo_2" value="<?php echo esc_attr($values['hero_titulo_2'] ?: 'Cristiano'); ?>" placeholder="Cristiano">
        </div>

        <h4>👦 Sección Adolescentes</h4>
        <div class="cfc-metabox-field">
            <label for="adol_titulo">Título</label>
            <input type="text" id="adol_titulo" name="adol_titulo" value="<?php echo esc_attr($values['adol_titulo'] ?: 'Adolescentes'); ?>" placeholder="Adolescentes">
        </div>
        <div class="cfc-metabox-field">
            <label for="adol_desc">Descripción</label>
            <textarea id="adol_desc" name="adol_desc" placeholder="Descripción del grupo..."><?php echo esc_textarea($values['adol_desc']); ?></textarea>
        </div>
        <div class="cfc-metabox-field">
            <label for="adol_edad">Rango de Edad</label>
            <input type="text" id="adol_edad" name="adol_edad" value="<?php echo esc_attr($values['adol_edad'] ?: '13-17 años'); ?>" placeholder="13-17 años">
        </div>
        <div class="cfc-metabox-field">
            <label for="adol_horario">Horario</label>
            <input type="text" id="adol_horario" name="adol_horario" value="<?php echo esc_attr($values['adol_horario'] ?: 'Sábados 4:00 PM'); ?>" placeholder="Sábados 4:00 PM">
        </div>
        <div class="cfc-metabox-field">
            <label for="adol_imagen">URL de Imagen</label>
            <input type="url" id="adol_imagen" name="adol_imagen" value="<?php echo esc_attr($values['adol_imagen']); ?>" placeholder="https://images.unsplash.com/...">
        </div>

        <h4>🚀 Sección Jóvenes</h4>
        <div class="cfc-metabox-field">
            <label for="jov_titulo">Título</label>
            <input type="text" id="jov_titulo" name="jov_titulo" value="<?php echo esc_attr($values['jov_titulo'] ?: 'Jóvenes'); ?>" placeholder="Jóvenes">
        </div>
        <div class="cfc-metabox-field">
            <label for="jov_desc">Descripción</label>
            <textarea id="jov_desc" name="jov_desc" placeholder="Descripción del grupo..."><?php echo esc_textarea($values['jov_desc']); ?></textarea>
        </div>
        <div class="cfc-metabox-field">
            <label for="jov_edad">Rango de Edad</label>
            <input type="text" id="jov_edad" name="jov_edad" value="<?php echo esc_attr($values['jov_edad'] ?: '18-30 años'); ?>" placeholder="18-30 años">
        </div>
        <div class="cfc-metabox-field">
            <label for="jov_horario">Horario</label>
            <input type="text" id="jov_horario" name="jov_horario" value="<?php echo esc_attr($values['jov_horario'] ?: 'Viernes 7:00 PM'); ?>" placeholder="Viernes 7:00 PM">
        </div>
        <div class="cfc-metabox-field">
            <label for="jov_imagen">URL de Imagen</label>
            <input type="url" id="jov_imagen" name="jov_imagen" value="<?php echo esc_attr($values['jov_imagen']); ?>" placeholder="https://images.unsplash.com/...">
        </div>
        <p class="description">Las secciones de Horarios, Reflexiones y CTA usan datos de CFC Opciones y CPTs.</p>
    </div>
    ...
    jQuery(document).ready(function($) {
        function showTemplateFields() {
            ...
            // Hide all
            $('.cfc-template-fields').removeClass('active');
            $('#cfc-no-template').show();

            // Show relevant
            if (template.indexOf('dar') !== -1) {
                $('#cfc-fields-dar').addClass('active');
                $('#cfc-no-template').hide();
            } else if (template.indexOf('visitanos') !== -1) {
                $('#cfc-fields-visitanos').addClass('active');
                $('#cfc-no-template').hide();
            } else if (template.indexOf('quienes-somos') !== -1) {
                $('#cfc-fields-quienes-somos').addClass('active');
                $('#cfc-no-template').hide();
            } else if (template.indexOf('inicio') !== -1) {
                $('#cfc-fields-inicio').addClass('active');
                $('#cfc-no-template').hide();
            }
        }
        ...
        // Classic editor
        $('#page_template').on('change', showTemplateFields);

        // Gutenberg - subscribe to changes
 ...
 */
function cfc_page_template_save($post_id) {
    if (!isset($_POST['cfc_page_template_nonce']) || !wp_verify_nonce($_POST['cfc_page_template_nonce'], 'cfc_page_template_save')) return;
...
add_action('save_post', 'cfc_page_template_save');

function cfc_evento_metabox_html($post) {
    ...
    <div class="cfc-metabox-field">
        <label for="registro_url">URL de Registro / Formulario</label>
    ...
    <div class="cfc-metabox-field">
        ...
        <p class="description">Si lo dejas vacío, dirá "Registrar Ahora". Ejemplos: "Confirmar Asistencia", "Inscribirme", "Más Info"</p>
    </div>
    <?php
}
...
function cfc_evento_save($post_id) {
    ...
    $fields = array('fecha_evento', 'hora_evento', 'ubicacion_evento', 'registro_url', 'texto_boton');
    foreach ($fields as $field) {
        if (isset($_POST[$field])) {
...
/**
 * Metabox HTML for Ministerios
 */
function cfc_ministerio_metabox_html($post) {
    ...
    ?>
    <div class="cfc-metabox-field">
        <label for="lider_ministerio">Líder del Ministerio</label>
    ...
    <div class="cfc-metabox-field">
        <label for="imagen_url">URL de Imagen (si no usa imagen destacada)</label>
        <input type="url" id="imagen_url" name="imagen_url" value="<?php echo esc_attr($imagen_url); ?>" placeholder="https://images.unsplash.com/...">
        <p class="description">Puedes usar una URL de Unsplash o subir una imagen destacada arriba.</p>
    </div>
    <?php
}
...
function cfc_ministerio_save($post_id) {
    ...
    $fields = array('lider_ministerio', 'horario_reunion', 'imagen_url');
    foreach ($fields as $field) {
        if (isset($_POST[$field])) {
            update_post_meta($post_id, $field, sanitize_text_field($_POST[$field]));
        }
...
/**
 * Metabox HTML for Reflexiones
 */
function cfc_reflexion_metabox_html($post) {
    ...
    <div class="cfc-metabox-field">
        ...
        <select id="tipo_reflexion" name="tipo_reflexion" style="width: 100%; padding: 8px;">
    ...
    <div class="cfc-metabox-field">
        <label for="podcast_url">URL del Podcast (Spotify, Apple, etc.)</label>
        <input type="url" id="podcast_url" name="podcast_url" value="<?php echo esc_attr($podcast_url); ?>" placeholder="https://open.spotify.com/episode/...">
    </div>
    <div class="cfc-metabox-field">
        <label for="archivo_url">URL del Archivo (PDF, documento)</label>
    ...
    <div class="cfc-metabox-field">
        ...
        <input type="text" id="duracion" name="duracion" value="<?php echo esc_attr($duracion); ?>" placeholder="Ej: 45 min, 5 min lectura">
    </div>
    <div class="cfc-metabox-field">
        <label for="serie">Serie / Tema</label>
        <input type="text" id="serie" name="serie" value="<?php echo esc_attr($serie); ?>" placeholder="Ej: Encuentros, La Familia, Fe Activa">
        <p class="description">Nombre de la serie de mensajes (opcional)</p>
    </div>
    <div class="cfc-metabox-field">
        <label for="episodio">Número de Episodio</label>
        <input type="text" id="episodio" name="episodio" value="<?php echo esc_attr($episodio); ?>" placeholder="Ej: 24">
        <p class="description">Para podcasts (opcional)</p>
    </div>
    <?php
}
...
function cfc_reflexion_save($post_id) {
    ...
    $fields = array('tipo_reflexion', 'video_url', 'podcast_url', 'archivo_url', 'duracion', 'serie', 'episodio');
    foreach ($fields as $field) {
        if (isset($_POST[$field])) {
            update_post_meta($post_id, $field, sanitize_text_field($_POST[$field]));
        }
...
/**
 * Metabox HTML for Equipo
 */
function cfc_equipo_metabox_html($post) {
    ...
    <div class="cfc-metabox-field">
        ...
        <input type="text" id="cargo" name="cargo" value="<?php echo esc_attr($cargo); ?>" placeholder="Ej: Pastor Principal, Líder de Alabanza">
    </div>
    <div class="cfc-metabox-field">
        <label for="icono">Icono (emoji)</label>
        <input type="text" id="icono" name="icono" value="<?php echo esc_attr($icono); ?>" placeholder="Ej: ✝️ 🎵 🚀 🎈 💪 👑">
        <p class="description">Usa un emoji que represente el ministerio</p>
    </div>
    <div class="cfc-metabox-field">
        <label for="color">Color del badge</label>
        <select id="color" name="color" style="width: 100%; padding: 8px;">
    ...
    <div class="cfc-metabox-field">
        ...
        <p class="description">Los números más bajos aparecen primero</p>
    </div>
    <?php
}
